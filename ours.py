...
class FedSystem(object):
    ...
    def server_excute(self):
        ...
        for r in range(start, args.n_round):
            ...
            for client_idx in client_idx_list:
                c_weight = self.client_update(client_idx, round_num, args.lr)
                # 记录训练损失和权重
    ...
    def cal_kl(self, idx):
        mu = torch.cat([param.flatten() for _, param in self.client_model_set[idx].named_parameters()], dim=0).to(
            self.args.device)
        omega = torch.cat([param.flatten() for param in self.client_omega_set[idx].values()], dim=0).to(
            self.args.device)
    ...
    def update_server_info(self):
        ...
        self.server_dist = D.Normal(mu,
                                    F.softplus(1 / (omega + self.args.eps)).clamp_max(100))  # todo 限制标准差最大为100（是否合适）
...
if __name__ == '__main__':
    ...
    args.root = f'results/0330/diri{args.data_split}/ours/'
    main(args)

# 0330 运行结果
# todo 1. data_split = 0 data = cifar10
# todo 2. data_split = 2 data = cifar10
